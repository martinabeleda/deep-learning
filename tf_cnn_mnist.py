...
MODEL_DIR = "/mnt/DATA/data_martin/models/hackdays/mnist_cnn"


def cnn_model_fn(features, labels, mode):
    """ Create tensorflow graph for CNN model """
    with tf.device("/gpu:2"):
        ...
        tf.logging.error(f"Invalid mode: {mode}")
...
def main(unused_argv):
    ...
    tf.logging.info(f"Training data shape: {train_data.shape}, Training label shape: {train_labels.shape}")
    tf.logging.info(f"Testing data shape: {test_data.shape}, Testing label shape: {test_labels.shape}")
